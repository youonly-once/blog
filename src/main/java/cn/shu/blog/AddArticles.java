...
public class AddArticles {
    ...
    public void setSwfToolsPath(String swfToolsPath) {
        AddArticles.swfToolsPath = swfToolsPath;
    }
    ...
    public static void addFileInfor(String path,String docdir){
        //获取该路径下的所有文件
        ...
        for (File file :
                files) {
            if (file.isFile()){
               /* new Thread(() ->*/ createSql(path,file)/*).start()*/;
            }
        }
    }
    ...
    private static void createSql(String path,File file){
        ...
        //源文件目录
        String sourceFilePath="/"+ext+"/"+filename+fileType;
        ...
        String desc="暂无描述";
        if("docx".equalsIgnoreCase(ext) || "doc".equalsIgnoreCase(ext)){
            desc=readWord(path+sourceFilePath).trim();
        }else{
            desc=HtmlDocument.readHtml(path+sourceFilePath,200).trim();
        }
